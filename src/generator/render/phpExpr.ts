/**
 * Pure IR expression → PHP text / multiline layout helpers.
 */
import type { Arg, Expr } from '../ir/types.ts';
import { type PhpLine, line, shiftLines } from './context.ts';
...
export function preferMultiline(expr: Expr): boolean {
  switch (expr.kind) {
    case 'and':
    case 'or':
      return (
...
export function renderJunctionLines(
 ...
 */
export function renderExprLayout(
  expr: Expr,
  depth: number,
  opts: RenderPhpOptions,
  groupParens = true,
